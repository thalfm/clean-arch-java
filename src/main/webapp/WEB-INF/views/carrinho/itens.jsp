	...
	<header id="layout-header">
		<div class="clearfix container">
			...
			<div id="header-content">
				<nav id="main-nav">
					<ul class="clearfix">
						<li><a href="${s:mvcUrl('SCCC#itens').build() }" rel="nofollow">Carrinho (${carrinho.quantidade })</a></li>
	...
	<section class="container middle">
		...
		<table id="cart-table">
				...
				<c:forEach items="${carrinho.itens }" var="item">
					<tr>
						...
						<td class="quantity-input-cell">
								<input type="number" min="0" id="quantidade" name="quantidade" value="${carrinho.addQuantidade(item) }" />
						</td>
						<td class="numeric-cell">${carrinho.getTotal(item) }</td>
						<td class="remove-item">
							<form action="${s:mvcUrl('SCCC#remover').arg(0, item.produto.id).arg(1,item.tipoPreco).build() }" method="post">
			...
			<tfoot>
				<tr>
					<td colspan="3">
						<form action="${s:mvcUrl('SPC#finalizar').build()}" method="post">
		...
		<h2>
			<a href="${s:mvcUrl('SPC#listar').build()}">Veja todos os livros que publicamos!</a>
		</h2>
	</section>
